      ...
      <el-container style="height: 650px; border-radius: 20px;">
        <el-aside width="300px" style="background-color:#3d3d3d;">
          <div class="friends">
            <div class="my-info">
              <img
                :src="userList.author.author_avatar"
                v-if="userList.author"
                alt=""
              />
              <span class="username" v-if="userList.author">{{
                userList.author.author_username
              }}</span>
            </div>
            <ul class="friend-list">
              <li
                class=""
                v-for="(item, index) in userList.userLists"
                :key="index"
                @click="toogleUser(item.target_id, item.target_username)"
              >
                <img
                  :src="item.target_avatar"
                  v-if="item.target_avatar"
                  alt=""
                />
        ...
        <el-container>
          <el-header
            style="background-color: #fff; border-bottom: 1px solid #eaeaea"
          >
            <div class="title">
            ...
            <ul class="content" ref="inner">
              ...
              >
                ...
                <div
                  :class="{
                    'content-info': true,
                    active: currentTargetId === item.target_id,
                  }"
                >
                  ...
                  <span
                    ...
                    v-show="currentTargetId === item.target_id"
                    @click="deleteLetter(item.id)"
                  ></span>
                </div>
              </li>
...
Vue.use(new VueSocketio({
  debug: true,
  connection: socketio('http://192.168.37.30:8081') 
}));
export default {
  sockets: {
    connection(data) {
  ...
  methods: {
    ...
    // 接受好友发来的消息
    receiveWord(){
       this.$socket.on('word',(word,fromid) => {
         console.log(word,fromid);
         this.getLetter()
       })
    },
    ...
    sendWord() {
      ...
      speakPublic(this.param).then((res) => {
        if (res.data.statusCode === 200) {
          this.param.word = "";
          this.$socket.emit("speak",this.param.word,localStorage.userId,this.param.hisId)
          this.getLetter();
        } else {
          this.$message({
            type: "error",
    ...
    // 获取用户列表
    getUserList() {
      getLetterList(this.param)
        .then((res) => {
          if (res.data.statusCode === 200) {
            let data = res.data.data;
            data.author.author_avatar =
              upload.UPLOAD.BASEURL + data.author.author_avatar;
