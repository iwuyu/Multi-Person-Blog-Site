<template>
  <div class="replay_or_publish">
    ...
    <div class="compile">
      <textarea class="txtarea" maxlength="200" cols="100%" rows="5" v-model="formComment.content" placeholder="你想说些什么呢..."></textarea>
    ...
    <div class="show_comment">
      ...
      <div class="comment_list" v-show="comment.length">
        <div class="comment_item" v-for="(item,index) in comment" :key="'comment_item'+index">
          <div class="comment">
          <div class="avatar" v-if="item.avatar"><img :src="item.avatar" alt=""></div>
          <div class="profile">
            <div class="name">{{item.username}}</div>
            <div class="time">{{item.time}}</div>
            <div class="content">{{item.content}}</div>
          </div>
          <div class="reply_btn" @click="commentClick(item.user_id,item.id,1)">回复</div>
          </div>
          <div class="reply">
            <div class="comment_list">
              <div class="comment_item" v-for="(value,key) in item.reply" :key="key">
                <div class="avatar" v-if="value.avatar"><img :src="value.avatar" alt=""></div>
                <div class="profile">
                  <div class="name">{{value.son_user}}</div>
                  <div class="time">{{value.time}}</div>
                  <div class="content"><span class="parent_name">@ {{value.parent_user}} </span> {{value.content}}</div>
                </div>
                <div class="reply_btn" @click="replyClick(item.id,value.user_id,value.id,2)">回复</div>
              </div>
            </div>
          </div>
        </div>
          ...
          if(res.data.statusCode === 200){
            ...
            if(this.formComment.content){
              ...
              if(!this.formComment.isReply){
                ...
                leaveComment(this.formComment).then(res => {
